...
class YahtzeeBot::MessageHandler
  CATEGORY_NAMES = {
  ...
  class << self
    ...
    def create_new_game(bot, chat_id, game)
      ...
      @current_game = Yahtzee::Game.new(chat_id:)
    ...
    def join_game(bot, chat_id, game, name, user)
      unless game
        bot.api.send_message(chat_id:, text: 'Сначала создайте игру с помощью /new')
        return
      end
      ...
      bot.api.send_message(chat_id:, text: "Ошибка: #{e.message}")
    end
    ...
    def roll_dice(bot, chat_id, game)
      return unless valid_game_state?(bot, chat_id, game)
      ...
      text += if game.dice.can_roll?
                "Осталось перебросов: #{game.dice.rolls_left}\n" \
                  'Можете перебросить кубики: /reroll 1 3 5'
              else
                "Перебросов не осталось!\n" \
    ...
    def select_category(bot, chat_id, game, category)
      return unless valid_game_state?(bot, chat_id, game, check_dice: true)

      unless category.between?(1, 13)
        bot.api.send_message(chat_id:, text: 'Категория должна быть от 1 до 13')
        return
      end
    ...
    def show_table(bot, chat_id, game)
      unless game
        bot.api.send_message(chat_id:, text: 'Нет активной игры')
        return
      end
    ...
    def show_leaderboard(bot, chat_id, persistence)
      ...
      if leaders.empty?
        bot.api.send_message(chat_id:, text: 'Пока нет данных для таблицы лидеров')
        return
      end
    ...
    def send_help(bot, chat_id)
      ...
      bot.api.send_message(chat_id:, text: default_help_text)
    end
    ...
    def send_rules(bot, chat_id)
      ...
      bot.api.send_message(chat_id:, text: default_rules_text)
    end
    ...
    def valid_game_state?(bot, chat_id, game, check_dice: false)
      ...
      if game.state != :in_progress
        bot.api.send_message(chat_id:, text: 'Игра ещё не началась!')
        return false
      end

      if check_dice && !game.dice.rolled?
        bot.api.send_message(chat_id:, text: 'Сначала бросьте кубики с помощью /roll')
        return false
      end
